...
data = {
    ...
    # },
    "online-lru": {
        "tps": [],  #np.asarray([7940.85, 15071.07]),
        "avg_lat": [], #np.asarray([8.0588, 8.4918]),
        "hr": [], # np.asarray([21.46]),
        "avg_mlat": [], # np.asarray([0.0523, 0.0510]),
    },

    "online-rgc": {
        "tps": [], # np.asarray([14530.72, 28878.35]),
        "avg_lat": [], # np.asarray([4.4038, 4.4310]),
        "hr": [], # np.asarray([57.32, 57.28]),
        "avg_mlat": [] #np.asarray([0.0585, 0.0639]),
    }
}

st = 6
ed = 11
threads = [2 ** k for k in range(st, ed)]

def parse_data_from_log(file_path):
    f = open(file_path)
    # lines = f.readlines()
    for thread in threads:
        for policy in ['lru', 'rgc']:
            ...
            print(lines[1])
            avg_lat = float(lines[1].split(' ')[7])
            tps = float(lines[1].split(' ')[15])
            hr = float(lines[1].split(' ')[23][:-1])
            avg_mlat = float(lines[1].split(' ')[9])
            data[f'online-{policy}']['tps'].append(tps)
            data[f'online-{policy}']['avg_lat'].append(avg_lat)
            data[f'online-{policy}']['hr'].append(hr)
            data[f'online-{policy}']['avg_mlat'].append(avg_mlat)

def P1():
    parse_data_from_log('local/log1031_fi.txt')

    fig = plt.figure(figsize=(14,7))
    ax = fig.subplots(2, 2)
    ax1 = ax[0][0]
    ax1.plot(threads, data["online-rgc"]["tps"], label="RGC", marker='o', linestyle='-')
    ax1.plot(threads, data["online-lru"]["tps"], label="LRU", marker='s', linestyle=':')
    ax1.set_xscale('log')
    ax1.set_yscale('log')
    ...
    ax2 = ax[0][1]
    ax2.plot(threads, data["online-rgc"]["avg_lat"], label="RGC", marker='o', linestyle='-')
    ax2.plot(threads, data["online-lru"]["avg_lat"], label="LRU", marker='s', linestyle=':')
    ax2.set_xscale('log')
    # ax2.set_yscale('log')
    ...
    ax3 = ax[1][0]
    ax3.plot(threads, data["online-rgc"]["hr"], label="RGC", marker='o', linestyle='-')
    ax3.plot(threads, data["online-lru"]["hr"], label="LRU", marker='s', linestyle=':')
    ax3.set_xscale('log')
    ax3.set_yscale('log')
    ...
    ax4 = ax[1][1]
    ax4.plot(threads, data["online-rgc"]["avg_mlat"], label="RGC", marker='o', linestyle='-')
    ax4.plot(threads, data["online-lru"]["avg_mlat"], label="LRU", marker='s', linestyle=':')
    ax4.set_xscale('log')
    # ax4.set_yscale('log')
