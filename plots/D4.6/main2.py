...
if __name__ == "__main__":
    ...
    ax.bar(X, lru_tps, width=0.3, label='Memcached', hatch='///', color=colors[5], edgecolor='black')
    ax.bar(X + 0.3, rgc_tps, width=0.3, label='MemHC', hatch='\\\\\\', color=colors[4], edgecolor='black')
    ax.set_xticks(X + 0.15)
    ax.set_xticklabels(threads_list)
    # ax.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
    ax.set_xlabel('#Threads', fontsize=12)
    ax.set_ylabel('Throughput (M requests/s)', fontsize=12)
    ax.set_title('(a) Throughput', y=-0.34)
    ...
    ax.bar(X, lru_lat, width=0.3, label='Memcached', hatch='///', color=colors[5], edgecolor='black')
    ax.bar(X + 0.3, rgc_lat, width=0.3, label='MemHC', hatch='\\\\\\', color=colors[4], edgecolor='black')
    ax.set_xticks(X + 0.15)
    ax.set_xticklabels(threads_list)
    ax.set_xlabel('#Threads', fontsize=12)
    ax.set_ylabel('Average Latency (ms)', fontsize=12)
    ax.set_title('(b) Latency', y=-0.34)
    ax.tick_params(axis='both', which='major', labelsize=12)
    ax.legend(fontsize=12)
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.24)
    plt.savefig('plots/D4.6/5.eps')
    plt.savefig('plots/D4.6/5.png')
