...
#ifndef CACHE_PERFORMANCE_INSIGHT_DYNAMIC_DECAY_LRU_H
#define CACHE_PERFORMANCE_INSIGHT_DYNAMIC_DECAY_LRU_H

#include "def.h"
class DDLruEntry {
public:
    DDLruEntry(Key key, double value, int ts_last_access, int ts_last_update): key(key), value(value) {
...
public:
    ...
    double GetValue(Key key) { update_value(*index_[key]); return index_[key]->value; }
    Key Pop();
...
#endif //CACHE_PERFORMANCE_INSIGHT_DYNAMIC_DECAY_LRU_H
