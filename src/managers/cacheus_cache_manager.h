...
#include "data_structures/heap_dict.hpp"


class CacheusCacheManager: public CacheManager {
public:
    class CacheusEntry {
        public:
            CacheusEntry() = default;
            CacheusEntry(Key key1, int32_t freq1, int32_t time1, int32_t is_new1 = true) {
                key = key1;
                freq = freq1;
                time = time1;
                is_new = is_new1;
    ...
    class CacheusLearningRate {
        public:
            ...
            double updateInDeltaDirection(double learning_rate_diff, double hitrate_diff) {
            ...
            void update(int32_t time) {
                if (time % period_len == 0) {
                    ...
                    double delta_LR = Round(learning_rate_curr, 3) - Round(learning_rate_prev, 3);
                    double delta = updateInDeltaDirection(delta_LR, hitrate_diff);

                    if (delta > 0) {
                        learning_rate = std::min(learning_rate + fabs(learning_rate * delta_LR), 1.0);
                        hitrate_nega_count = 0;
                        hitrate_zero_count = 0;
                    } else if (delta < 0) {
                        learning_rate = std::max(learning_rate - fabs(learning_rate * delta_LR), 0.001);
                        hitrate_nega_count = 0;
                        hitrate_zero_count = 0;
            ...
            double learning_rate, learning_rate_reset, learning_rate_curr, learning_rate_prev, hitrate{};
    ...
    CacheusCacheManager(int32_t buffer_size): CacheManager(buffer_size), lr(buffer_size) {
...
private:
    ...
    int32_t GetChoice() {
        return (rand() % 10000 / 10000.0) < W[0] ? 0 : 1;
    }
