...
#ifndef CACHE_PERFORMANCE_INSIGHT_DLIRS_CACHE_MANAGER_H
#define CACHE_PERFORMANCE_INSIGHT_DLIRS_CACHE_MANAGER_H
...
#include "data_structures/deque_dict.hpp"

class DLIRSCacheManager: public CacheManager {
public:
    class DLIRSEntry {
    public:
        DLIRSEntry() = default;
        DLIRSEntry(Key key1, bool is_lir1, bool is_demoted1 = false, bool in_cache1 = true) {
            key = key1;
            is_lir = is_lir1;
            is_demoted = false;
            in_cache = in_cache1;
    ...
    DLIRSCacheManager(int32_t buffer_size, double limit_ratio = 100.0f) : CacheManager(buffer_size), limit_ratio_(limit_ratio) {
        hirs_ratio = 0.01;
    ...
    void AdjustSize(bool h) {
        if (h) {
            hirs_limit = std::min(buffer_size_ - 1, hirs_limit + std::max(1, (int)((double)demoted / nonresident + 0.5)));
            lirs_limit = buffer_size_ - hirs_limit;
        } else {
            lirs_limit = std::min(buffer_size_ - 1, lirs_limit + std::max(1, (int)((double)nonresident / demoted + 0.5)));
    ...
    void LimitStack() {
        while (hirs_count + lirs_count + nonresident > limit_ratio_ * buffer_size_) {
    ...
    int32_t  hirs_limit, lirs_limit, hirs_count, lirs_count, demoted, nonresident;
    DEDequeDict lirs, hirs, q;
    int32_t time;
};

#endif //CACHE_PERFORMANCE_INSIGHT_DLIRS_CACHE_MANAGER_H
