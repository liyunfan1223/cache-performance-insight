...
#include "rgc4_cache_manager.h"

RC RGC4CacheManager::get(const Key &key) {
    ts_++;
...
RC RGC4CacheManager::put(const Key &key, const Value &value) {
    return RC::UNIMPLEMENT;
}

std::string RGC4CacheManager::get_name() {
    return {"RGC4"};
}
