...
#include "rgc3_cache_manager.h"

RC RGC3CacheManager::get(const Key &key) {
    ts_++;
...
RC RGC3CacheManager::put(const Key &key, const Value &value) {
    return RC::UNIMPLEMENT;
}

std::string RGC3CacheManager::get_name() {
    return {"RGC"};
}
