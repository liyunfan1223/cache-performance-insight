...
#include "drrip_cache_manager.h"

RC DRRIPCacheManager::get(const Key &key) {
    int new_bitmap;
...
std::string DRRIPCacheManager::get_name() {
    return {"SRRIPCacheManager"};
}

RC DRRIPCacheManager::put(const Key &key, const Value &value) {
    return RC::UNIMPLEMENT;
}

RC DRRIPCacheManager::check_consistency() {
    if (u_map_.size() > buffer_size_) {
        return RC::FAILED;
    }
    return RC::UNIMPLEMENT;
