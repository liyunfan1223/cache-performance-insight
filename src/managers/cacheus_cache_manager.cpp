...
#include "cacheus_cache_manager.h"

RC CacheusCacheManager::get(const Key &key) {
    bool miss = false;
    Key evicted = -1;
    ts += 1;
    lr.update(ts);
...
RC CacheusCacheManager::put(const Key &key, const Value &value) {
    return RC::UNIMPLEMENT;
}

std::string CacheusCacheManager::get_name() {
    return {"CACHEUS"};
}

void CacheusCacheManager::HitInS(Key key) {
...
void CacheusCacheManager::HitInQ(Key key) {
...
void CacheusCacheManager::AdjustSize(bool hitInQ) {
    if (hitInQ) {
        dem_count = std::max(1, dem_count);
        s_limit = std::min(buffer_size_ - 1, s_limit + std::max(1, (int)(((float)nor_count / dem_count) + 0.5)));
        q_limit = buffer_size_ - s_limit;
    } else {
        nor_count = std::max(1, nor_count);
        q_limit = std::min(buffer_size_ - 1, q_limit + std::max(1, (int)(((float)dem_count / nor_count) + 0.5)));
        s_limit = buffer_size_ - q_limit;
    }
}

Key CacheusCacheManager::Evict() {
...
void CacheusCacheManager::AddToHistory(CacheusCacheManager::CacheusEntry x, int32_t policy) {
...
Key CacheusCacheManager::HitInLRUHist(Key key) {
    Key evicted = -1;
...
Key CacheusCacheManager::HitInLFUHist(Key key) {
    Key evicted = -1;
...
Key CacheusCacheManager::Miss(Key key) {
    Key evicted;
    int32_t freq = 1;
