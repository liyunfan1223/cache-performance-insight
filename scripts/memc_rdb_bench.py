...
def bench(thread_num, value_size, memcached_mem, trace_file, memc_suffix, early_stop=True, threads_sync=True, has_warmup=True):
    print(f"Started. Args: threads-{thread_num} value_size-{value_size} memc_suffix-{memc_suffix} trace-{trace_file} earle_stop-{early_stop} has_warmup-{has_warmup}")
    ...
    if len(errors) > 0:
        ...
        try:
            ...
            results.append(log)
            print("result: ", log)
            with open("local/logs0419.txt", "a") as f:
                f.write(f"{log}\n")
...
def bench_memtier( memcached_mem, memc_suffix, threads, data_size):
    ...
    with open("local/logstier.txt", "a") as f:
        f.write(f"{args}\n")
        for o in outputs:
            f.write(f"{o}\n")
        f.write("\n")
...
if __name__ == "__main__":
    ...
    # for threads in [128, 32, 16, 4, 64, 8, 1, 2]:
    #     bench(threads, 1024, 7680, "DS1", "lru", False)
    #     bench(threads, 1024, 7680, "DS1", "rgc-DS1", False)
